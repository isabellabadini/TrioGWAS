...
wide <- offtest %>%
  left_join(patest, by=c("FID","p_IID")) ## yay this worked
wide <- wide %>%
...
#Splitting 'ped' data in half to test if this improves computational speed - not really..
#n <- nrow(ped)
#half_index <- floor(n/2)
#ped2 <- ped[(half_index+1):n,]
#ped <- ped[1:half_index,]
...
for (i in 1:length(offspring_snp_cols)) {
  ...
  for (j in seq_along(formulas)) {
    ...
    tryCatch({
      ...
      model <- lm(as.formula(formulas[[j]]), data = ped)

      robust_vcov <- vcovCL(model, cluster = ped$FID)
      coeftest_result <- coeftest(model, robust_vcov)
      ...
      temp_results[[results_index]] <- list(
        ...
        Beta = coeftest_result[1:4, "Estimate"],
        SE = coeftest_result[1:4, "Std. Error"],
        P_value = coeftest_result[1:4, "Pr(>|t|)"]
      )
