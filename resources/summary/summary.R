...
age_variable <- grep(paste0("^Age_", phen), colnames(cov), value = TRUE)

if(length(age_variable)==0){ 
   warning(paste("No corresponding age variable found for", phen))
   next
   }

merge <- merge(ph, cov, by = "IID")

ph2 <- subset(merge, select=c("IID", phen, "Sex", age_variable,
                        "PC1", "PC2", "PC3", "PC4", "PC5",
